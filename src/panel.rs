...
//!
//! The histogram bins RAW SENSOR COUNTS — pre-matrix, pre-debayer, black-subtracted, and VIEW-LIVE like the chart: every frame, the visible display pixels map back through the orientation bridge to their sensor tiles and every raw sample in those tiles bins into its CFA channel. The live EV gain rides the remap (RawView::spread), so the slider moves the histogram exactly as it moves the display — 2× per stop in linear-x, a one-stop translation in log-x, overspill collapsing into the clip spike — the same raw counts, still no curve. The x-axis is the display range (0 = black level, right edge = display white = sensor saturation at EV 0) in LINEAR counts, or log2 STOPS (0 at saturation down to −bitdepth) behind an explicit toggle — a labelled remap, never a silent curve. Bins run at [`HIST_OVERSAMPLE`]× the display width; stop gridlines XOR whole vertical hairlines into the OVERSAMPLED columns, so the box-average renders them sub-pixel. Bar heights are oriel's: percentile-normalized counts (87.5th percentile of the nonzero bins → 1/8 height), √-compressed VERTICALLY, additive RGB channels composing toward white.
//!
...
impl PanelTools {
    ...
    /// Re-sample the navigator thumbnail from (re-encoded) display pixels — cheap, so it tracks exposure changes live. The histogram tracks exposure too, but through its own gain remap of the raw counts at render (it never reads these display pixels).
